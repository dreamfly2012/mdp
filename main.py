...
class MarkdownPreviewer(tk.Frame):
    ...
    def create_widgets(self):
        ...
        self.preview_text = HTMLScrolledText(self)
        self.preview_text.grid(row=2, column=2, sticky="nsew", padx=5, pady=5)
        scrollb = tk.Scrollbar(self, command=self.preview_text.yview)
        scrollb.grid(row=2, column=3, sticky='nsew')
        self.preview_text['yscrollcommand'] = scrollb.set

        # 创建底部按钮
        self.button_frame = tk.Frame(self)
        self.button_frame.grid(row=3, column=0, columnspan=3, sticky="w")
        self.preview_button = tk.Button(self.button_frame, text="预览", command=self.preview)
    ...
    def preview(self):
        ...
        print(html)
        self.preview_text.set_html(html)
